...
for w in wounds:
    ...
    all_meshes = []  
    all_meshes += glob.glob(f'{w}/*.ply')

    points = np.loadtxt(f'{w}/PolyLine_Large.txt')
    poly_large = points


    for reference_mesh_path in all_meshes:
    
        reference_mesh = o3d.io.read_triangle_mesh(reference_mesh_path)

        all_mesh_sfm = copy.deepcopy(all_meshes)
        all_mesh_sfm.remove(reference_mesh_path)

        sampled_points_GT = reference_mesh.sample_points_uniformly(number_of_points=NB_POINTS_SAMPLED, use_triangle_normal=True)
        sampled_points_GT = compute_polygon_intersection_pc(sampled_points_GT,poly_large)
        ...
        for sfm_mesh_path in tqdm(all_mesh_sfm, desc='measuring error', total=len(all_mesh_sfm)):
            ...
            metrics['Camera'] = sfm_mesh_path.split('/')[-1].split('.')[0].split('_')[-2].lower()
            metrics['Reference Method'] = reference_mesh_path.split('/')[-1].split('.')[0].split('_')[0]
            metrics['Reference Camera'] = reference_mesh_path.split('/')[-1].split('.')[0].split('_')[-2].lower()
            metrics['Wound'] = w
...
total_metrics.to_csv(f'metrics_precision_NAngelo.csv')
