...
def collate_image_dataset_batch(batch: Dict, num_rays_per_batch: int, keep_full_image: bool = False):
    ...
    elif "sampling_masks" in batch:        
        # sampling_weights = batch["sampling_masks"][..., 0].to(device).view(-1)
        # sampling_weights = torch.cumsum(sampling_weights, dim=0)        
        # indices = torch.rand(num_rays_per_batch, device=device) * sampling_weights[-1]
        # indices = torch.searchsorted(sampling_weights, indices)
        # indices = unravel_index(indices, batch["sampling_masks"].shape[:-1])

        nonzero_indices = torch.nonzero(batch["sampling_masks"][..., 0].to(device), as_tuple=False)
        chosen_indices = random.sample(range(len(nonzero_indices)), k=num_rays_per_batch)
        indices = nonzero_indices[chosen_indices]
        
        # (Very slow implementaion)
        # sampling_weights = batch["sampling_masks"][..., 0].to(device)
        # img_samp = torch.multinomial(sampling_weights.sum(dim=[1,2]), num_samples=num_rays_per_batch, replacement=True)
        # img_samp = torch.bincount(img_samp, minlength=sampling_weights.shape[0])

        # indices_list = []
        # for b_idx in range(img_samp.shape[0]): 
        #     num_rays_per_image = img_samp[b_idx]
        #     if num_rays_per_image > 0: 
        #         indices = torch.multinomial(sampling_weights[b_idx].view(-1), num_samples=num_rays_per_image, replacement=True)
        #         indices = unravel_index(indices + b_idx*np.prod(sampling_weights.shape[1:]), sampling_weights.shape)
