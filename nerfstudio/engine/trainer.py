...
class Trainer:
    ...
    def __init__(self, config: cfg.Config, local_rank: int = 0, world_size: int = 1):
        ...
        # Slow down printing
        self.config.logging.steps_per_log = 1000
        ...
        with TimeWriter(writer, EventName.TOTAL_TRAIN_TIME):
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                if step_check(step, self.config.logging.steps_per_log, run_at_zero=True):
                    ...
                    writer.put_dict(name="Train Losses", scalar_dict=loss_dict, step=step)
                    writer.put_dict(name="Train Metrics", scalar_dict=metrics_dict, step=step)
    ...
    def eval_iteration(self, step):
        ...
        if step_check(step, self.config.trainer.steps_per_eval_image, run_at_zero=self.config.trainer.sanity_check):
            ...
            eval_loss_dict, metrics_dict, images_dict = self.pipeline.get_eval_image_metrics_and_images(step=step)
            # writer.put_time(
            ...
            #     avg_over_steps=True,
            # )
            writer.put_dict(name="Eval Losses", scalar_dict=eval_loss_dict, step=step)
            writer.put_dict(name="Eval Metrics", scalar_dict=metrics_dict, step=step)
            group = "Renderings"
            for image_name, image in images_dict.items():
                if image_name == "img" or image_name == "mse":
                    writer.put_image(name=group + "/" + image_name, image=image, step=step)
                else:
                    writer.put_image(name="Misc/" + image_name, image=image, step=step)
