...
class SurfaceModel(Model):
    ...
    def get_loss_dict(self, outputs, batch, step: int, metrics_dict=None) -> Dict:
        loss_dict = {}
        ...
        if self.training:
            ...
            if step > 500:
                loss_dict["eikonal_loss"] = ((grad_theta.norm(2, dim=-1) - 1) ** 2).mean() * self.config.eikonal_loss_mult/2
            else:
    ...
    def get_image_metrics_and_images(
        ...
        # MSE color map
        mse = (rgb - image) ** 2
        mse = colormaps.apply_colormap(mse)
        ...
        import numpy as np
        mse = np.exp(-0.1 * np.log(10.) * psnr.item())

        # all of these metrics will be logged as scalars
        metrics_dict = {"psnr": float(psnr.item()), "ssim": float(ssim), "mse": float(mse)}  # type: ignore
